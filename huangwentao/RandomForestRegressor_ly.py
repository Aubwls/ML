...
rf_params = {
    'n_estimators': 180,           # 略微减少树的数量
    'max_depth': 18,               # 略微增加深度以提高精度
    'min_samples_split': 3,        # 减少分割样本数，增加模型灵活性
    'min_samples_leaf': 1,         # 减少叶节点样本数
    'max_features': 'sqrt',        # 使用sqrt特征数，原始成功设置
    'bootstrap': True,
    'random_state': 217,
    'n_jobs': -1,                  # 使用所有CPU核心
    'verbose': 1,
...
if hasattr(rf_model, 'oob_score_'):
    print(f"袋外分数 (OOB Score): {rf_model.oob_score_:.6f}")
...
result_df.to_csv("result_RF_LY.csv", index=False)
print("\n随机森林结果已保存到: result_RF_LY.csv")

# 计算平均误差 - 使用原始代码的计算方式
data = pd.read_csv("result_RF_LY.csv")
column3 = data.iloc[:, 2]
