...
for i, column in enumerate(columns):
    ...
    # 创建并训练GCForest模型 - 修改参数以减少内存使用
    gcf = gcForest(
        shape_1X=shape_1X,
        n_mgsRFtree=20,  # 减少树的数量
        window=[2],  # 使用更小的窗口
        stride=1,
        cascade_test_size=0.2,
        n_cascadeRF=2,
        n_cascadeRFtree=30,  # 减少树的数量
        cascade_layer=3,  # 减少级联层数
        min_samples_mgs=0.1,
        min_samples_cascade=0.05,
        tolerance=0.001,
        n_jobs=-1  # 减少并行工作数以避免内存竞争
    )

    gcf.fit(X_train.values, y_train_discrete)

    # 分批预测以避免内存溢出
    batch_size = 500  # 调整批次大小
    n_samples = X_test.shape[0]
    pred_proba_all = []

    for start_idx in range(0, n_samples, batch_size):
        end_idx = min(start_idx + batch_size, n_samples)
        X_batch = X_test.values[start_idx:end_idx]

        # 预测当前批次
        pred_proba_batch = gcf.predict_proba(X_batch)
        pred_proba_all.append(pred_proba_batch)

    # 合并所有批次的预测结果
    pred_proba = np.vstack(pred_proba_all)
