...
# 使用MultiOutputRegressor包装GradientBoostingRegressor模型以支持多输出回归
# 设置与XGBoost类似的参数
model_adj = MultiOutputRegressor(GradientBoostingRegressor(
    n_estimators=200,
    learning_rate=0.1,
    max_depth=5,
    min_samples_split=5,
...
result_df.to_csv("result_GBRT.csv", index=False)

print("<*>"*50)

# 读取结果并计算平均误差
data = pd.read_csv("result_GBRT.csv")
column3 = data.iloc[:, 2]
